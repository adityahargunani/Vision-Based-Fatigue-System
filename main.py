...
# For Alarms
mixer.init()
head_down_alarm_path = "siren-alert-96052.mp3"
eyes_closed_alarm_path = "sound.wav"
yawning_alarm_path = "music.wav"

def head_down_alarm():
    mixer.music.load(head_down_alarm_path)
    mixer.music.play()
...
while True:
    ...
    for rect in rects:
        ...
        xx = euler_angle[0, 0]
        cv2.putText(frame, "X: " + "{:7.2f}".format(xx), (20, 290), cv2.FONT_HERSHEY_SIMPLEX,
                    0.75, (0, 0, 0), thickness=2)
        yy = euler_angle[1, 0]
        cv2.putText(frame, "Y: " + "{:7.2f}".format(yy), (20, 320), cv2.FONT_HERSHEY_SIMPLEX,
                    0.75, (0, 0, 0), thickness=2)
        zz = euler_angle[2,0]
        cv2.putText(frame, "Z: " + "{:7.2f}".format(zz), (20, 350), cv2.FONT_HERSHEY_SIMPLEX,
                    0.75, (0, 0, 0), thickness=2)
        ...
        if xx > 10: 
            ...
            s = threading.Thread(name='head_down_alarm', target=head_down_alarm)
            s.start()
